<template>
  <ValidationObserver ref="requestForm" v-slot="{errors, invalid, validated}">
    <section :class="['request account-receivable-confirm-create', {validated: validated}, {invalid: invalid}]">
        ...
        <div class="request-header-container">
          ...
          <div class="request-header_status">
            <div>
              ...
              <span v-show="mode == 'edit'"><br/>（編集中）</span></div>
          </div>
        </div>
      </div>
        ...
        <collapse>
          ...
          <template v-slot:collapse-body>
            <b-row class="px-4 py-1 mt-3">
              ...
              <b-col v-if="item.requestModel.paymentMethod == 'その他'" md="4" class="pl-0 other-payment-method">
                <InputSskf
      ...
      ></CommentConfirmRequest>
      <NormalProcessingDialog v-model="showNormalDialog" title="確認しました。"></NormalProcessingDialog>
  ...
  methods:{
    ...
    async validateRequest() {
      ...
      for(let i = 0; i< listNames.length; i++) {
          if(this.item[listNames[i]]) {
            listRowsNeedChecked= listRowsNeedChecked.concat(filter(this.item[listNames[i]], data => {
      ...
      listRowNeedCheckDiffHasComment = filter(listRowsNeedChecked, data => {
        return data.mishuLimit < data.accruedUnitPrice && data.isChecked != 1 && 
          (!data.commentDetailsRequests || data.commentDetailsRequests.length == 0);
      });
