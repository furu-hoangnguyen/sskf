    ...
    <section :class="['request account-receivable-apply', {validated: validated}, {invalid: invalid}]">
      ...
      <div class="request-footer btnForRequest">
        <template v-if="mode != 'edit'">
          ...
          <b-button v-if="statusPage == 5 &&
            (!item.requestModel.requestedAt || item.requestModel.requestedAt == '') &&
            loginUser &&
            loginUser.roles &&
            loginUser.roles.includes('applyPersons')"
            class="btn-white"
            variant="none"
          ...
          <b-button v-if="(item.requestModel.requestedAt &&
            item.requestModel.requestedAt != '' &&
            statusPage == 5 &&
            loginUser &&
            loginUser.roles &&
            loginUser.roles.includes('applyPersons')
            ...
            class="btn-white"
            variant="none"
            @click="validateCorrectionFromSalesRepresentativeBeforeRequest"
          >
            営業担当者に<br/>修正依頼する
          </b-button>
          <b-button
      ...
      ></CommentConfirmRequest>
      <CommentConfirmRequest
        v-model="isShowSalesChargeModal"
        :request-comment="commentForRequestSalesCharge"
        :is-apply="false"
        title="指定した担当者に、修正依頼します"
        btn-submit-label="修正依頼する"
        @cancel="closeSalesChargeModal"
        @submit="requestCorrectionFromSalesRepresentative"
      ></CommentConfirmRequest>
      <NormalProcessingDialog v-model="showNormalDialog" :title="normalTitleDialog"></NormalProcessingDialog>
      ...
      <DeleteDialog v-if="showPopupDelete" v-model="showPopupDelete" title="申請情報を削除しますか？" @delete="deleteRequest">
      </DeleteDialog>
    </section>
  </ValidationObserver>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      isShowSalesChargeModal: false,
      normalTitleDialog: "申請しました。",
      commentForApply: "",
      commentForRequestSalesCharge: "",
      labelBtnCommentModal: "",
      salesChargeEditItemRequests: []
    };
  },
  computed: {
    isCanApply() {
      return this.statusPage == 5 && 
        (
          !this.item.requestModel.requestedAt ||
          this.item.requestModel.requestedAt == "" ||
          (this.item.requestModel.requestedAt &&
            this.item.requestModel.requestedAt != "" &&
            this.loginUser &&
  ...
  methods: {
    ...
    addCommentIntoSalesChargeEditItemRequests(inputParams){
      inputParams = Array.isArray ? inputParams : [inputParams];
      inputParams.forEach(input => {
        this.salesChargeEditItemRequests.forEach(editItem => {
          if(input.accountReceivableDetailCd == editItem.accountReceivableDetailCd) {
            editItem.commentDetailsRequest = input.comment;
          }
        });
      });
    },
    ...
    closeSalesChargeModal(comment) {
      this.commentForRequestSalesCharge = comment;
      this.isShowSalesChargeModal = false;
    },
    closeConfirmCommentPopup(comment, isApply = false) {
    ...
    requestCorrectionFromSalesRepresentative(comment) {
      let salesChargeEditRequest = {
        comment: comment,
        requestCd: this.item.requestModel.cd,
        salesChargeEditItemRequests: this.salesChargeEditItemRequests
      }
      AccountReceivableCreateService.requestSalesChargeEdit(salesChargeEditRequest)
        .then(response => {
          this.showNormalDialog = true;
        })
        .catch(err => {
    ...
    deleteRequest() {
      RequestService.deleteRequest(this.item.requestModel.cd)
      .then(response => {
        this.returnToList();
      })
      .catch(err => {
